...
SELECT  Country,                   REPLACE( Country , '?', ' '),
        ...
        `Area`,                    REPLACE(`Area` , '?', ' '),
        `Location`,                REPLACE(`Location` , '?', ' /*1.*/  /* These Query below shows and removes the extra character '?' in the columns of the attacks table. 
 The extra character '?' appears at the end of some columns and inside some misspelled words. 
 Update the misspelled words before you do the general update if not, it would be difficult to identify the misspelled words.
 The extra character '?' is lowest common factor of the misspelled words*/
...
UPDATE sakila.attacks 
SET `Location` =  'Vung Tàu'
WHERE `Location` =  'V?ng T#Query To Check For `Reported` In The Date Column  
Select * 
From sakila.attacks 
Where `Date` like '%Reported%' 
Order By `Date` desc ;
...
/* Query to check for '?' in Name columns*/ 
...
/* Query To Update the Name Column with right data */
...
/* Query to check for '?' in Species columns*/ 
...
/* Query to check for '?' in `Age` columns*/ 
SELECT  *
FROM sakila.attacks 
WHERE `Age` like  '%?%' ;


/* Query To Remove '?' from the `Time` Column.*/
SELECT  *
FROM sakila.attacks 
WHERE `Time` like  '%?%';


/* Query To Remove '?' from the Injury Column.*/
SELECT  *
FROM sakila.attacks 
WHERE Injury like  '%?%';
